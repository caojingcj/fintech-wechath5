...
(function () {
    ...
    app.controller('returnCtrl', ['$window', '$rootScope', '$scope', '$state', '$http', 'REST', 'RS', function ($window, $rootScope, $scope, $state, $http, REST, RS) {
        $('body').loading({
            title: '请稍等',
            name: 'test1',
            discription: '加载中订单中'
        ...
        if (!parameter.loginFlag) {
            ...
            setTimeout(function () {    // 直接去我要还款页面
                removeLoading('test1');
                $state.go('app.return', {openId: parameter.openId})
            }, 2000);
        }
    }])
})();